...
import { DateTime } from 'luxon'; // --- 1. ADDED IMPORT ---

// --- 2. ADDED TYPE from events.tsx ---
type GroupedEvents = {
  'Within 3 days': Event[];
  'Within 1 week': Event[];
...
    }, [events, currentUser]);
    ...
    // --- 3. ADDED GROUPING LOGIC from events.tsx ---
...
    }, [events]);
    ...
    // --- 4. ADDED HELPER FUNCTIONS from events.tsx ---
    const formatDate = (dateString: string, timezone: string) => {
...
                ) : (
                    <>
                    {/* This is your existing "You coming?" section */}
                        <View className="mb-8">
...
                        {/* --- 5. ADDED UPCOMING EVENTS SECTION --- */}
                        <View>
                            <Text className="text-3xl font-bold text-gray-800 px-2 mb-2">Upcoming Events</Text>
                            {events?.length === 0 && (
                                <View className="bg-white p-5 my-2 rounded-lg items-center">
                                    <Text className="text-base text-gray-500">You have no upcoming events.</Text>
                                </View>
                            )}
