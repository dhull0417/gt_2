...
const CreateGroupPopup: React.FC<CreateGroupPopupProps> = ({ onClose }) => {
  ...
  const handleCreateGroup = () => {
    ...
    const variables = { name: groupName, time: meetTime, schedule };
    mutate(variables, {
      onSuccess: () => {
        onClose();
      },
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      {/* Main container: Sets the size, shape, and padding of the popup. */}
      <View className="bg-white rounded-xl w-11/12 max-w-lg shadow-lg relative max-h-[90%] p-6">
        ...
        {/* ScrollView contains all the form content, making it scrollable within the parent View. */}
        <ScrollView
          style={{ width: '100%' }}
          showsVerticalScrollIndicator={false}
          keyboardShouldPersistTaps="handled" // Improves interaction with buttons when keyboard is open
        >
          ...
          {/* The Create Group button is now safely inside the scrollable area */}
          <TouchableOpacity
          ...
          {/* The Cancel button is also inside */}
