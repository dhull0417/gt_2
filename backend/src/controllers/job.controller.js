...
export const regenerateEvents = asyncHandler(async (req, res) => {
  ...
  const expiredEvents = allEvents.filter(event => {
    // --- THIS IS THE FIX ---
    // Add a guard clause to safely skip any events with missing or malformed time data.
    if (!event.date || !event.time || typeof event.time !== 'string' || !event.time.includes(':')) {
      console.warn(`Skipping malformed event with ID: ${event._id}`);
      return false; // Exclude this malformed event from processing
    }
